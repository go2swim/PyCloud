...
class DropboxTokenManager:
    ...
    def authentication(self):
        ...
        # 2. Формирование ссылки на авторизацию
        auth_url = (
        ...
        # 3. Запуск локального сервера для перехвата кода
        self.start_server()

        # 4. Открытие браузера с авторизационным URL
        webbrowser.open(auth_url)
        print("Ожидание авторизации...")

        # 5. Ждём завершения работы сервера (используем флаг)
        while not self.server_shutdown:
            pass

        # 6. Обмен кода на токен
