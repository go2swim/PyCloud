...
class DropBox(CloudInterface):
    ...
    def handle_response(self, response, retry_on_401=True):
        ...
        if 200 <= response.status_code < 300:
            ...
            # Обработка ошибок
            try:
                # Пытаемся получить описание ошибки из JSON-ответа
                error_info = response.json()
                error_message = error_info.get('error_summary', 'Нет описания ошибки')
            except ValueError:
                # Если JSON невалиден, используем текст ответа
    ...
    def delete(self, path):
        """Удаление папки или файла на Dropbox."""
        try:
    ...
    def list_folder(self, path):
        ...
        response = requests.post(url, headers=self.headers.headers, data=json.dumps({"path": path}))
    ...
    def get_cloud_tree(self, folder_name, tree_list, root):
        ...
        if folder_name == '':
            root = ''
        items = self.list_folder(f'/{self.ROOT_FOLDER}/{root.replace(os.path.sep, "/")}')
        if not items:
            return
    ...
    def upload_dir_on_cloud(self, upload_folders):
        ...
        for folder_dir in upload_folders:
            ...
            for file_name in files:
                ...
                self.upload_file(path_to_file_os, f'/{self.ROOT_FOLDER}/{path_to_file_os.replace(os.path.sep, "/")}')
    ...
    def get_os_and_clouds_files(self, folder_dir, full_path):
        ...
        items = self.list_folder(f'/{self.ROOT_FOLDER}/{folder_dir.replace(os.path.sep, "/")}')
        if not items:
            return os_files, []
    ...
    def get_data_for_comparison(self, os_path_file, cloud_file):
        ...
        # Дата модификации файла в облаке
        cloud_modified_time = datetime.datetime.fromisoformat(cloud_file['server_modified'])

        # Хэш файла в ОС
        with open(os_path_file, 'rb') as f:
            os_file_md5 = hashlib.md5(f.read()).hexdigest()

        # Хэш файла в облаке (content_hash)
        ...
        for folder_dir in exact_folders:
            ...
            for cloud_file in refresh_files:
                ...
                if os_modified_time > cloud_modified_time:
                    self.upload_file(os_path_file, cloud_file['path_display'], replace=True)

            for remove_file in remove_files:
                self.delete(remove_file['path_display'])

            # Загружаем новые файлы в Dropbox
    ...
    def update_dir_on_pc(self, exact_folders):
        for folder_dir in exact_folders:
            ...
            for cloud_file in refresh_files:
                ...
                if os_modified_time < cloud_modified_time:
                    self.download(full_path, cloud_file['path_display'], is_folder=False)
                    print(f'Файл {cloud_file['name']} обновлён в папке {os.path.basename(full_path)}')

            for remove_file in remove_files:
                os.remove(f'{full_path}{os.path.sep}{remove_file}')

            # Скачиваем новые файлы с Dropbox
            for clouds_download_file in download_files:
                self.download(full_path, clouds_download_file['path_display'],  is_folder=False)
    ...
    def list_files(self, path):
        ...
        for item in items:
            result.append(FileData(
                ...
                item_modified=datetime.datetime.fromisoformat(item.get('server_modified'))
                if item.get('server_modified') else None
            ))

        return result

    def downloading_folders(self, download_folders):
        """Скачивает папки с Dropbox."""
        ...
        for clouds_folder in download_folders:
            # Получаем локальный путь, куда нужно скачивать папку
            from src.clouds_manager import get_os_path_by_cloud_path
            root_path = get_os_path_by_cloud_path(clouds_folder)  # D:\...\SyncFolder\clouds_folder

            # Проверяем, существует ли папка на диске, если да — пропускаем
            if os.path.exists(root_path):
                continue

            # Скачиваем папку с Dropbox
            self.download(os.path.sep.join(root_path.split(os.path.sep)[:-1]),
    ...
    def download(self, downloaded_path, save_path, is_folder=False):
        ...
        if is_folder:
            ...
            local_folder_path = os.path.join(downloaded_path, os.path.basename(save_path))
...
if __name__ == '__main__':
    dropbox = DropBox('', '')
    dropbox.download(r'D:\Document\SecCourseMATMEX\Python\PyCloud', '/SYNC_FOLDERS/testovaya papka/1', True)
    # dropbox_token_manager = DropboxTokenManager()
    # dropbox_token_manager.refresh_token()
