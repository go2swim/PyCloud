...
class TestDropboxMethods(unittest.TestCase):
    ...
    def setUp(self):
        # Мокаем DropboxHeadersManager, чтобы избежать вызовов аутентификации
        self.mock_headers_manager = patch('src.Dropbox.dropbox.DropboxHeadersManager').start()

        # Создаем фейковые токены и хедеры для замены настоящих
        self.mock_headers_manager.return_value.headers = {
            "Authorization": "Bearer FAKE_TOKEN",
            "Content-Type": "application/json"
        }

        # Также можно замокать свойства token и token_manager
        self.mock_headers_manager.return_value.token_manager = MagicMock()
        self.mock_headers_manager.return_value.token = 'FAKE_TOKEN'

        # Создаем тестовый объект DropBox с замоканным headers
    ...
    @patch('src.Dropbox.dropbox.requests.post')
    @patch('src.Dropbox.dropbox.DropboxHeadersManager')
    def test_handle_response_401_refresh(self, mock_headers_manager, mock_post, requests_mock):
        ...
        # Mock token refresh and retry response
        mock_response_200 = MagicMock()
        mock_response_200.status_code = 200
        mock_response_200.json.return_value = {"success": True}

        mock_headers_manager.return_value.token_manager.refresh_token.return_value = None
        mock_post.return_value = mock_response_200

        requests_mock.return_value = mock_response_200

        # Test retrying on 401 error
    ...
    def test_upload_file_success(self, mock_headers_manager, mock_post, mock_file):
        ...
        # Call upload_file
        self.dropbox.upload_file("testfile.txt", "/path_in_dropbox/testfile.txt", replace=True)

        # Assertions
        mock_post.assert_called_once_with(
            f"{DROPBOX_CONTENT_URL}/files/upload",
            headers={'Authorization': 'Bearer FAKE_TOKEN', 'Content-Type': 'application/octet-stream',
    ...
    def test_create_folder_success(self, mock_headers_manager, mock_post):
        ...
        mock_post.assert_called_once_with(
            ...
            headers={'Authorization': 'Bearer FAKE_TOKEN', 'Content-Type': 'application/json'},
            data=json.dumps({"path": "/path_in_dropbox/testfolder", "autorename": False})
        )
    ...
    def test_upload_dir_on_cloud_success(self, mock_post, mock_walk):
        ...
        # Проверка, что метод был вызван дважды (для обоих файлов)
    ...
    def test_get_os_and_clouds_files(self, isfile_mock, listdir_mock, list_folder_mock):
        ...
        # Вызов функции
        os_files, cloud_files = self.dropbox.get_os_and_clouds_files('test_folder', '/fake/path')
    ...
    def test_update_dir_on_cloud(self, get_data_for_comparison_mock, get_os_and_clouds_files_mock, delete_mock,
                                 upload_file_mock):
        """Test updating directory on cloud."""
        # Мокаем файлы на ОС и в облаке
        get_os_and_clouds_files_mock.return_value = (
            ['file1.txt', 'file2.txt'],  # os_files
            [
        ...
        # Мокаем сравнение данных
        get_data_for_comparison_mock.side_effect = [
            (10, 9, 'fake_md5_os', 'fake_md5_cloud'),  # file2.txt needs upload
        ]
        ...
        # Проверка
        upload_file_mock.assert_called_with(f'{os.path.sep}test_folder{os.path.sep}file1.txt',
    ...
    @patch('src.Dropbox.dropbox.DropBox.get_data_for_comparison')
    @patch('os.remove')
    def test_update_dir_on_pc(self, remove_mock, get_data_for_comparison_mock, get_os_and_clouds_files_mock,
                              download_mock, _):
        ...
        get_data_for_comparison_mock.side_effect = [
            (8, 9, 'fake_md5_os', 'fake_md5_cloud'),  # file2.txt needs download
        ]
        ...
        # Проверка
        download_mock.assert_called_with('test_folder', '/dropbox/path/file3.txt', is_folder=False)
    ...
    def test_list_files(self, post_mock, handle_response_mock):
        ...
        # Мокаем ответ от API Dropbox
        post_mock.return_value = MagicMock()
        handle_response_mock.return_value = {
            'entries': [
                {'.tag': 'file', 'name': 'file1.txt', 'size': 1000, 'server_modified': '2022-01-01T12:00:00Z'},
        ...
        # Проверка результата
        expected_result = [
            FileData(item_type="FILE", item_name="file1.txt", item_size=1000,
                     item_modified=datetime.datetime.fromisoformat("2022-01-01T12:00:00Z")),
            FileData(item_type="DIR", item_name="folder1", item_size=None, item_modified=None)
        ]
        self.assertEqual(result, expected_result)

        # Проверка вызова API с правильными аргументами
    ...
    def test_downloading_folders(self, get_os_path_mock, download_mock, exists_mock):
        ...
        # Мокаем получение пути на ПК
        get_os_path_mock.side_effect = lambda folder: f'/local/path/{folder}'

        # Мокаем существование пути на ПК
        exists_mock.side_effect = [False, True]  # Первую папку скачиваем, вторую пропускаем

        # Вызов функции
        download_folders = ['folder1', 'folder2/subfolder']
        self.dropbox.downloading_folders(download_folders)

        # Проверка вызова download только для первой папки
        download_mock.assert_called_once_with('', '/SYNC_FOLDERS/folder1', is_folder=True)
    ...
    def test_download_folder(self, makedirs_mock, list_folder_mock, download_file_mock):
        ...
        list_folder_mock.return_value = {
            'entries': [
                {'.tag': 'file', 'name': 'file1.txt', 'path_display': '/dropbox/file1.txt'},
                {'.tag': 'file', 'name': 'file2.txt', 'path_display': '/dropbox/file2.txt'}
            ]
        }

        # Вызов функции для загрузки папки
        self.dropbox.download('/local/path', '/dropbox/folder', is_folder=True)

        # Проверка, что была создана локальная папка
        makedirs_mock.assert_called_once_with(f'/local/path{os.path.sep}folder', exist_ok=True)

        download_file_mock.assert_any_call(f'/local/path{os.path.sep}folder', '/dropbox/file1.txt')
        download_file_mock.assert_any_call(f'/local/path{os.path.sep}folder', '/dropbox/file2.txt')
    ...
    @patch('src.Dropbox.dropbox.open', new_callable=mock_open)  # Для мока файловых операций
    @patch('src.Dropbox.dropbox.requests.post')
    @patch('src.Dropbox.dropbox.DropBox.handle_response')
    def test_download_file(self, handle_response_mock, post_mock, open_mock, basename_mock, join_mock):
        ...
        # Проверка вызова requests.post с корректными аргументами
        post_mock.assert_called_once_with(
        ...
        # Проверка записи содержимого в файл
        open_mock().write.assert_called_once_with(file_content)

        # Проверка, что функция вывела сообщение о успешной загрузке файла
    ...
    def test_download_file_failed(self, handle_response_mock, post_mock):
        ...
        # Мокаем неуспешный ответ от Dropbox API
        post_mock.return_value = MagicMock(status_code=404)

        # Вызов функции
        self.dropbox.download_file(downloaded_path, save_path)

        # Проверка, что handle_response был вызван для обработки ошибки
