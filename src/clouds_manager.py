...
CLOUDS = {
    "yandex": lambda folder_full_path: YandexDisk(
        os.path.basename(folder_full_path), folder_full_path
    ),
    "google": lambda folder_full_path: GoogleDrive(
        os.path.basename(folder_full_path), folder_full_path
    ),
    "dropbox": lambda folder_full_path: DropBox(
        os.path.basename(folder_full_path), folder_full_path
    ),
}
SAVE_SYNC_FILE = (
    r"save_sync_folder.txt"
...
def get_and_update_sync_folders():
    ...
    try:
        ...
        if len(existing_paths) < len(paths):
            print(
                f"Путь: {set(paths).difference(set(existing_paths))} больше не актуален, "
...
def sync_folders(list_clouds=[]):
    ...
    for get_cloud in get_clouds:
        ...
        for folder_full_path in sync_folders:
            ...
            exact_folders = list(
                set(os_tree_list).intersection(set(tree_list))
            )  # SYNC_FOLDER\1\2...
...
def sync_locals_folders(cloud_name):
    ...
    tree_list = [re.split(rf"{ROOT_FOLDER}\\", path)[-1] for path in tree_list]
...
def list_files(path, clouds: [str]):
    ...
    for get_cloud in get_clouds:
        ...
        if not file_data:
            return {}
        result[os.path.basename(path)][cloud.__class__.__name__] = cloud.list_files(
            path
        )
...
def get_os_path_by_cloud_path(clouds_path):
    ...
    root_path = sync_folder_this_last_folder.get(
        clouds_path.split(os.path.sep)[0], None
    )
    if root_path:
    ...
    return get_valid_folder_path(
        clouds_path
    )  # clouds_path всегда в таком случае просто название папки
